...
def data_transform_pipe(df_dict):
    ...
    if "pubmed" in df_dict.keys():
        df_dict["pubmed"]=extract_drug(df_dict["pubmed"],list_of_drug,"title",'pubmed')
    if "clinical_trials"in df_dict.keys():
        df_dict["clinical_trials"]=extract_drug(df_dict["clinical_trials"],list_of_drug,"title",'clinical_trials')
    df_final=merge_df(df_dict["pubmed"], df_dict["clinical_trials"], ["drug","journal"], ["drug","journal"],"outer",("pubmed","clinical_trials"))
    return df_final
