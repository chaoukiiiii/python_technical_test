...
def group_by_cols(df, cols_group, cols_grouped, index_col):
    df_out = (
        df.groupby(cols_group)[cols_grouped]
        .apply(lambda g: g.reset_index(drop=True).T.to_dict("dict"))
        .reset_index(name=index_col)
    )
    return df_out
...
def merge_df(df_right, df_left, col_right, col_left, how, suffixes):
    return df_left.merge(
        df_right, left_on=col_left, right_on=col_right, how=how, suffixes=suffixes
    )
